...
def draw_attributes(img_path, df):
    ...
    for row in df.iterrows():
        top, right, bottom, left = row[1][6:].astype(int)
        if row[1]['Male'] >= 0.5:
            gender = 'Male'
        else:
            gender = 'Female'

        age = np.argmax(row[1][1:6])
        text_showed = "{} {}".format(age, gender)
...
def main():
    ...
    parser.add_argument('--model', type=str,
                        default='afad_face_model.pkl', required = True,
                        help='path to trained model (default: afad_face_model.pkl)')
